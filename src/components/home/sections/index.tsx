...
const Section = ({category}: SectionProps) => {
  ...
  return (
    <div className='my-20'>
      <h2 className='uppercase text-[#B67253] text-right text-2xl leading-7 mb-6'>- {category}</h2>
...
const BigPicture = ({category, title, blurb, publishedDate, postId, thumbnailUrl}: BigProps) => {
  return (
    <div className='relative h-[820px] rounded-lg newsreader'>
      ...
      <div className="w-full h-full absolute top-0 left-0 z-20 px-12 pt-24 pb-16 rounded-lg text-white flex flex-col items-start justify-between bg-[#00000091]">
        <div className='w-full'>
          <h3 className='text-xl font-medium uppercase mb-6'>— {category}</h3>
          <h4 className='text-5xl font-bold italic w-1/2'>{title}</h4>
        </div>
        <div className='  w-full'>
